<template style="font-size: 28px">
    ...
    >
      <template v-slot:top>
        ...
        <v-toolbar flat>
          <v-toolbar-title>Principle</v-toolbar-title>
...
<v-col cols="12" sm="6" md="4">
      ...
      <template v-slot:no-data>
        <v-btn color="primary" @click="getPrincipleList">Reset</v-btn>
      </template>
...
CreatedDate:  new Date().toISOString().substr(0, 10),
  ...
  computed: {
    ...mapState({
      PrincipleList: state => state.Principlevue.PrincipleList,
      showSnackBar: state => state.status.showSnackBar,
      type: state => state. status.type,
      text: state => state. status.text,
      editedNo: state => state. Principlevue.editedNo
    }),
    formTitle() {
      return this.editedIndex === -1 ? "Add Principle" : "Edit Principle";
    },
    disabledSaveButton() {
      return !this.valid
    }
  ...
  methods: {
    ...mapActions({
      getPrincipleList: "Principlevue/getPrincipleList",savePrincipleList: "Principlevue/savePrincipleList", updatePrincipleList: "Principlevue/updatePrincipleList", deletePrincipleList: "Principlevue/deletePrincipleList"
    }),
    ...mapMutations({      
    ...
    deleteItemConfirm() {
      //this.PrincipleList.splice(this.editedIndex, 1);
      this.deletePrincipleList(this.editedIndex),
      this.closeDelete();
    },
    close() {
      this.dialog = false;
    ...
    save() {
      if (!this.$refs.form.validate()) {
        ...
        if (this.editedIndex ==-1) {
          this.savePrincipleList(this.editedItem);
          this.close();
          
        } else {
          this.updatePrincipleList(this.editedItem);
          this.close();
        }
      }
    },
